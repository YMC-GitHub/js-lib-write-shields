...
(function (global, factory) {
  ...
  (global = global || self, global['write-shields'] = factory());
  ...
  */
  ...
  var mkdirsSync = function _mkdirsSync(dirname) {
    // has dir?
  ...
  var startWithSmall = function (x) { return x.charAt(0).toLowerCase() + x.slice(1); };

  // exports.camelCase = x => x.charAt(0).toUpperCase() + x.slice(1).toLowerCase()
  var camelCase = function (str) { return startWithSmall(
      str
        .split(/[_.-]/)
        .map(function (x) { return x.charAt(0).toUpperCase() + x.slice(1).toLowerCase(); })
        .join('')
    ); };
  var funcName = camelCase;
  ...
  var rootPath = path$1.resolve(__dirname, '..');
